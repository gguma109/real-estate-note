...
if (!content.includes('id="tab-calc"')) {
    content = content.replace(gasBtnStr, gasBtnStr + calcBtnStr);
}
...
// To safely update switchTab, we will just replace the specific parts
if (content.includes('const gBtn = document.getElementById(\\'tab-gas\\');') && !content.includes('const cBtn = document.getElementById(\\'tab-calc\\');')) {
    ...
    // Add calc logic in switchTab
    const elseIfGasStr = "} else if (tab === 'gas') {";
    const elseIfCalcStr = `} else if (tab === 'calc') {
    ...
    content = content.replace(elseIfGasStr, elseIfCalcStr);
}
...
if (!content.includes('function calculateSettlement() {')) {
    ...
    const lastScriptIdx = content.lastIndexOf(endScriptMarker);
    if (lastScriptIdx !== -1) {
        content = content.substring(0, lastScriptIdx) + jsLogicStr + '\n    ' + content.substring(lastScriptIdx);
    }
}

fs.writeFileSync(file, content);
console.log('Successfully injected exact PRORATION logic and UI!');
